...
from streamtomocupy import cfunc_lprec
from streamtomocupy import cfunc_lprecfp16
...
class Padj:
    def __init__(
        self, fZ, lp2p1, lp2p2, lp2p1w, lp2p2w, C2lp1, C2lp2, cids, lpids, wids
    ):
        self.fZ = fZ
        self.lp2p1 = lp2p1
        self.lp2p2 = lp2p2
...
def create_gl(N, Nproj, Ntheta, Nrho):
    ...
    P = Pgl(Nspan, N, Nproj, Ntheta, Nrho, proj, s, thsp, rhosp, aR, beta, am, g, B3com)
    return P
...
def splineB3(x2, r):
    ...
    x = x2[
        int(cp.ceil((sizex + 1) / 2.0) - ri - 1) : int(cp.ceil((sizex + 1) / 2.0) + ri)
    ]
    d = cp.abs(x - x2c) / r
    B3 = x * 0
...
def create_adj(P):
    ...
    fZ = cp.fft.fftshift(
        fzeta_loop_weights_adj(P.Ntheta, P.Nrho, 2 * P.beta, P.g - np.log(P.am), 0, 4)
    )
    fZ = cp.ascontiguousarray(
        fZ[:, : P.Ntheta // 2 + 1] / (P.B3com[:, : P.Ntheta // 2 + 1])
    )
    const = (
        (P.N + 1) * (P.N - 1) / P.N**2 / 2 / np.sqrt(2) * np.pi / 6 * 0.86 * 4
    ...
    for k in range(0, P.Nspan):
        z1 = P.aR * (
            x1[cids] * cp.cos(k * P.beta + P.beta / 2)
            + x2[cids] * cp.sin(k * P.beta + P.beta / 2)
        ) + (1 - P.aR)
        z2 = P.aR * (
            -x1[cids] * cp.sin(k * P.beta + P.beta / 2)
    ...
    z2n = z2n / P.aR
    lpids = cp.where((z1 >= -P.beta / 2) & (z1 < P.beta / 2) & (abs(z2n) <= 1))[
        0
    ].astype("int32")
    ...
    z2n = z2n / P.aR
    lpidsw = cp.where(
        (z1[wids] >= -P.beta / 2) & (z1[wids] < P.beta / 2) & (abs(z2n) <= 1)
    )[0]
    # left side
    wids2 = cp.where(cp.log(z2) < cp.log(P.am) - P.g + (P.rhosp[1] - P.rhosp[0]))[
        0
    ].astype("int32")

    z2n2 = cp.exp(cp.log(z2[wids2]) - cp.log(P.am) + P.g) - (1 - P.aR) * cp.cos(
        z1[wids2]
    )
    z2n2 = z2n2 / P.aR
    lpidsw2 = cp.where(
        (z1[wids2] >= -P.beta / 2) & (z1[wids2] < P.beta / 2) & (abs(z2n2) <= 1)
    )[0]
    ...
    for k in range(P.Nspan):
        pids[k] = cp.where(
            (P.proj >= k * P.beta - P.beta / 2) & (P.proj < k * P.beta + P.beta / 2)
        )[0]
    ...
    for k in range(P.Nspan):
        lp2p1[k] = (lp2p1[k] - proj0[k]) / projl[k] * (len(pids[k]) - 1) + (
            proj0[k] - proj0[0]
        ) * projp
        lp2p1w[k] = (lp2p1w[k] - proj0[k]) / projl[k] * (len(pids[k]) - 1) + (
            proj0[k] - proj0[0]
        ) * projp
    ...
    Padj0 = Padj(fZ, lp2p1, lp2p2, lp2p1w, lp2p2w, C2lp1, C2lp2, cids, lpids, wids)
...
def fzeta_loop_weights_adj(Ntheta, Nrho, betas, rhos, a, osthlarge):
    ...
    thsplarge = (
        cp.linspace(-1 / 2, 1 / 2, Nthetalarge, endpoint=False, dtype="float32") * betas
    )
